...
if [[ "$TF_STAGE" == "stage1" ]]; then
  ...
  terraform -chdir=${INPUT_TF_STAGE} init -backend-config="key=${INPUT_STATE_KEY}.tfstate"
  terraform -chdir=${INPUT_TF_STAGE} import \
  ...
  azurerm_container_registry.rebeccajensen-acr \
  "/subscriptions/${INPUT_ARM_SUBSCRIPTION_ID}/resourceGroups/rg-rebeccajensen/providers/Microsoft.ContainerRegistry/registries/acrrebeccajensenacmp2400" || true
  terraform -chdir=${INPUT_TF_STAGE} import \
  -var="ARM_CLIENT_ID=${INPUT_ARM_CLIENT_ID}" \
  -var="ARM_CLIENT_SECRET=${INPUT_ARM_CLIENT_SECRET}" \
  -var="DJANGO_SECRET_KEY_PROD=${INPUT_DJANGO_SECRET_KEY_PROD}" \
  azurerm_container_group.aci-rebeccajensen-acmp \
  "/subscriptions/${INPUT_ARM_SUBSCRIPTION_ID}/resourceGroups/rg-rebeccajensen/providers/Microsoft.ContainerInstance/containerGroups/aci-rebeccajensen-acmp" || true
  terraform -chdir=${INPUT_TF_STAGE} apply -auto-approve \
  -var="ARM_CLIENT_ID=${INPUT_ARM_CLIENT_ID}" \
  -var="ARM_CLIENT_SECRET=${INPUT_ARM_CLIENT_SECRET}" \
  -var="DJANGO_SECRET_KEY_PROD=${INPUT_DJANGO_SECRET_KEY_PROD}"
